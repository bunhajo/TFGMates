...
Una medida de probabilidad es una función que asigna probabilidades a conjuntos (el espacio medible de probabilidades), en este caso a conjuntos de secuencias $\mathbb{X}$, definida como la menor colección de subconjuntos de $X^{\mathbb{Z}}$ tq:
...
2. Los elementos de $\mathbb{R^N}$ son secuencias semi-infinitas mientras que los de $X^\mathbb{Z}$ son, de hecho, secuencias bi-infinitas. Utilizaremos de nuevo un mapeado, esta vez biyectivo $h: \mathbb{N} \rightarrow \mathbb{Z}$. Consideraremos los dígitos en el orden $0, 1 , -1, 2, -2, \ldots$.
...
Aplicando TEK sobre las $\mu_n$'s, obtenemos una única $P'$ en $(\mathbb{R}^{\mathbb{N}}, R^{\mathbb{N}})$ que concuerda con $\mu_n$. Por lo tanto, si $A_i$ es una $\sigma$-álgebra, $\forall i \in \{1, \ldots, n \}$ y $A = A_1 \times \ldots \times A_n$, entonces existe una única probabilidad $P'$ que concuerda con $\mu_n$ 
$$
...
Sea $X$ una variable aleatoria en un espacio de probabilidad $(\Omega, F, P)$. Sea $E$ una sub $\sigma$-álgebra de $F$, o sea, $E$ una $\sigma$-álgebra y $E \subset F$.

**Definición.** La esperanza condicional de $X$ respecto a $E$ es cualquier variable aleatoria $Y$ que satisfaga

1. $Y$ es medible respecto a $E$
2. $\forall A \in E$,
...
Si $Z$ es una variable aleatoria en $(\Omega, F, P)$, entonces $Z$ induce una $\sigma$-álgebra $\sigma(Z)$ en $\Omega$, en concreto la menos $\sigma$-álgebra que contiene conjuntos de la forma $Z^{-1}(B)$,para conjuntos de Borel $B$. Sea $A$ un conjunto en $F$ con función característica $1_A$, tal y como hemos definido previamente. Si fijamos $a \in \mathbb{R}$ y evaluamos en $x \in Z^{-1}(a)$ la esperanza condicional $E(1_A|\sigma(Z))$, encontramos que

$$
E(1_A|\sigma(Z))(x)= P(x \in A| Z=a)
$$

Una filtración es un conjunto incremental de $\sigma$-álgebras $\{F\}= F_1 \subset F_2 \subset \ldots$. Por ejemplo, los conjuntos $\mathbb{H}_l$ en $(X^{\mathbb{Z}}, \mathbb{X}, P)$ son filtraciones.

**Definición.** Una secuencia $\{X\}=X_1, X_2 \ldots$ es una martingala respecto a la filtración $\{F\}$ si

1. $\forall i, X_i$ es medible respecto a $F_i$.
2. $\forall s, t \in \mathbb{N}$ tal que $t>s$, $X_s=E(X_t|F)$.
...
# tab$FECHA=as.Date(tab$FECHA. st)
...
$$
y(x)=g \big(\sum_{i=0}^n w_i x_i\big)
$$
donde $x$ es una neurona con $n$ inputs $x=(x_0, \ldots, x_n)$ y un axón de output $y(x)$. Los términos $w=w_0, \ldots, x_n$ son los pesos, y determinan cómo deben ser de intensificados los inputs.
