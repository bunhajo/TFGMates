...
\begin{tabular}{ll}
\textbf{\Large Director:} & \textbf{\Large Dr. Josep Vives Santa Eulàlia } \\
\textbf{\Large Realitzat a:} & \textbf{\Large  Departament de Probabilitat} \\ 
& \textbf{\Large Lògica, i Estadística}
\\
...
Este apartado estará dedicado a demostrar la unicidad de los procesos mediante el siguiente Teorema. Nos basamos en la demostración dada por Daniel Ray Upper en Theory and Algorithms for Hidden Markov Models
and Generalized Hidden Markov Models.
...
Utilizaremos el Teorema de Convergencia de Martingalas de Doob, sin entrar en demostraciones.

**Teorema.** Si $\{\mathcal{F_n}\}$ es una secuencia crecientes de $\sigma$-álgebras, y $X$ es una función medile tal que $\mathbb{E}(|X|) < \infty$, entonces $\lim_{n \rightarrow \infty} \mathbb{E}(X|\mathcal{F}_n)$ converve de fomar casi segura a $\mathbb{E}(X|\mathcal{F})$, donde $\mathcal{F}$ es la menor $\sigma$-álgebra que contiene todas las $\mathcal{F}_n$'s.

En nuestro caso, utilizamos la versión reducida del Teorema de Doob para el caso en que $X$ viene dada por la función indicadora $1_A$.

**Corolario.** Si $\mathcal{F}_n$ es una secuenciea creciente de $\sigma$-álgebras  y $A$ es un evento. Entonces $P(A|\mathcal{F}_n) \rightarrow P(A|\mathcal{F})$ casi seguramente, donde $\mathcal{F}$ es la menor $\sigma$-álgebra que contiene todas las $\sigma$-álgbras $\mathcal{F}_n$'s. *cqd*
...
El siguiente diagrama muestra la arquitectura general de un HMM de parámetros $\lambda_i$ con los dos procesos estocásticos asociados a este modelo.
...
El segundo procesos estocástico viene de que el valor de $O(t)$ depende de la variable oculta asociada $S(t)$.
...
$$
 N(x, \mu_i, \Sigma_i)= \frac{1}{\big(2 \pi\big)^{L/2}|\Sigma_i|^{1/2}}e^{(x-\mu_i)\Sigma_i^{-1}(x-\mu_i)^t},
$$ 

y por lo tanto 
$$
b_j(o(t)) = \sum\limits_{k=1}^K c_{jk}N(x, \mu_j, \Sigma_j).
$$
...
Una neurona artificial puede ser implementada de muchas maneras. La definición matemática general para una ANN de una sola capa, como la mostrada en la figura, es la siguiente:
...
$g$ es una función de activación que balancea cómo de potente debe ser el output (si existe) de la neurona, basándose en la suma de los input. En el caso de neuronas reales, $g$ es una función $\delta$ de Kronecker que determina una relación binaria de todo o nada para el output. Sin embargo, no se suele implementar de esa manera una neurona artificial, ya que como veremos luego, los algoritmos para entrenarlas suelen venir determinados por funciones diferenciables.
...
La función sigmoidal es una función real de variable real diferenciable. Si $y(x)= \frac{1}{1 + e^{-x}}$,

$$
\frac{dy(x)}{dx} = \frac{e^{-x}}{(1+e^{-x})^2} = y(x)(1-y(x))
$$

por lo que $y(x)$ tiene una derivada simple. En adelante, la función de activación que utilizaremos será la sigmoidal logística para $m=1$.


## Backpropagation

Entre las ANN's existentes, una de las más empleadas por su alta eficiencia es el Perceptrón Multicapa (MLP) mediante el uso del algoritmo de *Back-Propagation* descrito por Jian-Rong Chen . A diferencia del Perceptrón de una única capa, el MLP puede implementar gran variedad de funciones complejas, incluída la función XOR, que no puede ser realizada por el de una única capa como demostraron Minsky y Papert *(Perceptrons: an introduction to computational geometry is a book written by Marvin Minsky and Seymour Papert and published in 1969)*.
...
\begin{algorithm}
\begin{algorithmic}[1]
\REPEAT {\STATE computar gradiente $\frac{\partial \mathcal{E}}{\partial w}(t)$}
\FOR{todos los pesos y sesgos}
\IF{$\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1)\frac{\partial \mathcal{E}}{\partial w_{ij}}(t)>0$}
\STATE $\Delta_{ij}(t) = \textbf{ min}(\Delta_{ij}(t-1)\eta^+, \Delta_{max}$)
\STATE $\Delta w_{ij}(t) = -\textbf{ sign}\Big(\frac{\partial \mathcal{E}}{\partial w_{ij}}(t)\Big)\Delta_{ij}(t)$
\STATE $w_{ij}(t+1)=w_{ij}(t)+\Delta_{ij}(t)$
\STATE $\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1) = \frac{\partial \mathcal{E}}{\partial w_{ij}}(t)$
\ELSIF{$\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1)\frac{\partial \mathcal{E}}{\partial w_{ij}}(t)<0$}
\STATE $\Delta_{ij}(t) = \textbf{ maximum}(\Delta_{ij}(t-1)\eta^-, \Delta_{min})$
\STATE $\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1)=0$
\ELSIF{$\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1)\frac{\partial \mathcal{E}}{\partial w_{ij}}(t)=0$}
\STATE $\Delta w_{ij}(t) = -\textbf{ sign}\Big(\frac{\partial \mathcal{E}}{\partial w_{ij}}(t)\Big)\Delta_{ij}(t)$
\STATE $w_{ij}(t+1)=w_{ij}(t)+\Delta_{ij}(t)$
\STATE $\frac{\partial \mathcal{E}}{\partial w_{ij}}(t-1) = \frac{\partial \mathcal{E}}{\partial w_{ij}}(t)$
...
\centering
    \includegraphics[width=1\textwidth]{ub.png}
\label{fig:figure}
\end{minipage}
\end{table}
...
Un fenómeno que ocurre en el cerebro es la plasticidad temporal (STDP), donde la conexión de la sinapsis modula la probabilidad de que un evento pre-sináptico (acción sobre las dendritas) cause un evento post-sináptico (acción sobre los axones). En otras palabra, la conexión sináptica influencia a la hora de que un input sea o no procesado. De la misma forma en una ANN los pesos pueden ser modificados a partir de reglas que incorporen información de la sinapsis mediantes reglas como la STDP.

Básicamente, la regla de STDP dicta que si un evento presinaptico se lanza justo antes que uno postsináptico, entonces el peso de la sinapsis es reforzado. Si es al revés, esta conexión se debilita.
*Nessler et al.* desarrollaron una versión de STDP para computar el algoritmo de Maximización de la Esperanza en un circuito neuronal.
...
$$
g_k(t)=w_{k0}+\sum\limits_{i=1}^{N}\delta_i^k(t)w_{ki}(t)
$$
...
\delta_i^k(t) =
\begin{cases}
1 \quad \text{ si el input }i \text{ se activó en el intervalo }[t - \sigma, t]\\
0 \quad \text{ en caso contrario}
\end{cases}
$$
...
La idea de este modelo mixto es utilizar ANN's entrenadas específicamente para cada uno de los estados del HMM, que será de tipo Gaussiano (GHMM), de manera que la verosimilitud de cada estado para las observaciones vendrá determinado por los pesos de las redes neuronales.

![Caption for the picture.](GHMM-ANN_fig1.png)
...
$$
 N(x, \mu_i, \Sigma_i)= \frac{1}{\big(2 \pi\big)^{N/2}|\Sigma_i|^{1/2}}e^{(x-\mu_i)\Sigma_i^{-1}(x-\mu_i)^t},
$$ 

Por lo tanto, en un modelo mixto tendríamos $K$ mezclas $z_1, z_2, \ldots, z_K$ con $\sum z_k = 1$ y
...
$$
P(y)=\sum_{z}P(z)P(y|z)=\sum_{k=1}^K \pi_k N(y|\mu_k, \Sigma_k)\\
\text{donde } \sum_k \pi_k=1, 0\leq\pi_k\leq1
$$

Sea una muestra $y_r$ de $y$, definimos ahora $R(z_{kr})$ como

$$
P(z_k=1|y) = R(z_{kr})= \frac{\pi_k N(y_r|\mu_k,\Sigma_k)}{\sum\limits_{j=1}^K\pi_j N(y_r|\mu_j, \Sigma_j)}
$$
Para simplificar, supongamos que las muestras son independientes ($\Sigma=I$). 

$$
R(z_{kr})= \frac{\pi_k e^{-0.5(x-\mu_k)(x-\mu_k)^t}}{\sum\limits_{j=1}^K \pi_j e^{-0.5(x-\mu_j)(x-\mu_j)^t}}, \quad (1)
$$

Tendremos que $R(z_{kr})$ alcanza el máximo cuando $y_r=\mu_k$. Como hemos comentado antes, lo que hacemos es sustituir las distribuciones de los estados por Redes Neuronales, así que suponemos que para la neurona $k$, su peso $w_{ki}, i\in \{1, \ldots, N\}$ aproxima $\mu_k$ y, por lo tanto, podemos sustituir el componente $-0.5(x-\mu_k)(x-\mu_k)^t$ por el producto escalar entre $w$ e $y$, $w^ty$ de manera que, con esta reformulación, obtenemos

$$
R(z_{kr})=C\frac{\pi_k e^{w_k^ty_r}}{\sum\limits_{j=1}^K \pi_j e^{w_j^t y_r}} \quad (2)
$$
...
$$
\mu_k=w_k = \frac{\sum\limits_{s=1}^M R(z_{ks}y_s)}{\sum_{s=1}^M R(z_{ks})}, \quad (3)
$$
...
En este ANN, $k$ es la neurona de output que acaba de disparar y el cual se supone corresponde al $\mu_k$ del GHMM, y se actualiza con cada nueva muestra. Para calcular el $w_k$ final, lo que haremos será al valor $\mu_k$ que obtengamos, añadirle un término proveniente de aplicar una adaptación de la regla STDP de Nessler, teniendo en cuenta si ha habido señal presináptica en el input $i$ en el intervalo $[t-\sigma, t]$ de la siguiente manera:

$$
\Delta w_{ki} = 
\begin{cases}
e^{-w_{ki}+1}-1 \quad \text{si } \delta_i^k(t) = 1 \\ 
-1 \quad \quad \quad \quad \text{ }\text{si } \delta_i^k(t) = 0
\end{cases}
$$

con $i \in \{1, \ldots, N\}$. En el primer caso se refuerza la conexión, mientras que en el segundo se debilita.

Otor parámetro del GHMM es el coeficiente de mezcla $\pi_k$ obtenido en (4). El peso del sesgo $w_{k0}$, que asemejaría $\pi_k$ y representaría la media de veces que se activa la neurona $k$,  también está sujeto a las reglas STDP de la siguiente manera:

$$
\Delta w_{k0}= z_ke^{-w_{k0}+1}-1
$$

Y tendríamos $w_{k0}^{new}=w_{k0}+\Delta w_{k0}$. Para el resto de $w_{ki}$ la regla varía un poco, añadiéndose un término $\eta_k$ proporcional al número de veces que ha disparado la neurona $k$. Y tenemos   

$$
w_{ki}^{new} = w_{ki} + \eta_k \Delta w_{ki}
$$

Redefiniendo las $\pi_k$ como hemos visto en función del peso del sesgo tendríamos

$$
\pi_k^{ann}=\frac{e^{w_{k0}}}{\sum\limits_j e^{w_{j0}}}
$$

Introducimos este término en (2) para terminar de definir $R(z_{kr})$ en función únicamente de los pesos de la ANN.

$$
R(z_{kr})=C\frac{\frac{e^{w_{k0}}}{\sum\limits_j e^{w_{j0}}}e^{w_{k}^t y_r}}{\sum\limits_{l=1}^K\frac{e^{w_{l0}}}{\sum\limits_j e^{w_{j0}}}e^{w_{l}^t y_r}}
$$
$$
Pr_(z\text{ dispara}|y_r)=R(z_{kr})=C\frac{e^{w_{j0}+w_{k}^t y_r}}{\sum\limits_{l=1}^K e^{w_{l0}+w_{l}^t y_r}}
$$

Finalmente, extraemos la probabilidad de que el input sea del tipo que el ANN ha reconocido utilizando $g_k(t)$, que hemos visto en el apartado anterior, definida como
...
$$
Pr_{snn}(t)=\underset{k \in K}{max}\frac{e^{g_k(t)}}{Z}
$$
...
BaumWelch = function(returns, mu, sigma, 
  ...
  if(k!=1){
    ...
    # Verosimilitud para determinar convergencia del algoritmo
