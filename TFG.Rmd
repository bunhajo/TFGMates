...
1. Si $n$ par, entonces $h(n+1)<0, c(n+1)=c(n)-1, d(n+1)=d(n)$.
2. Si $n$ impar, entonces $h(n+1)>0, c(n+1)=c(n), d(n+1)=d(n)+1$.

\textbf{Demostración}

1. Si $n$ par, entonces $n=2*i$, para algún $i \in \mathbb{N} \textbackslash 0$, y:
...
2. Si $n$ impar, entonces $n=2i +1$, para algún $i \in \mathbb{N} \textbackslash \{0\}$, y:
