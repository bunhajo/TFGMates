...
\textbf{Teormea de Extensión de Kolmogorov}. Suponemos que nos dan un conjunto de medidas de probabilidad $\mu_n$ consitentes en $(\mathbb{R}, R^n)$, es decir que cumplen:
...
$$
No demostraremos el TEK directamente, sino que lo usaremos para demostrar una variante del mismo que se adapte a nuestras necesidades, teniendo en cuenta las diferencias que existen entre el espacio de probabilidad $(\mathbb{R}, R^n), P'$ y el de un proceso estacionario $(X^\mathbb{Z}, \mathbb{X}, P)$
...
$$
C(S)=\{x \in X^{\mathbb{Z}}|x_i \in S_i, i \in [a,b]\} = \{x \in X^{\mathbb{Z}}|x_{a-1} \in X, x_i \in S_i, i \in [a,b]\} = C(X \times S) 
$$ 
De la misma manera:
$$
C(S)=\{x \in X^{\mathbb{Z}}|x_i \in S_i, i \in [a,b]\} = \{x \in X^{\mathbb{Z}}|x_{b+1} \in X, x_i \in S_i, i \in [a,b]\} = C(S \times X) 
$$ 
2. Tenemos en cuenta $S \times X \subset X^{[a, b+1]}$:
$$
D(S \times X) =\{x \in X^{[a-1, b]} | x_b+1 \in X, x_i \in S_{i+1}, i+1 \in [a,b]\} =\{x \in X^{[a-1, b-1]} | x_i \in S_{i+1}, i+1 \in [a,b]\} \times X
$$
De forma semejante a lo visto anteriormente también tenemos:
$$
D(S \times X) =\{x \in X^{[a-2, b-1]} |x_{a-2} \in X, x_i \in S_{i+1}, i+1 \in [a,b]\} = X \times \{x \in X^{[a-1, b-1]} | x_i \in S_{i+1}, i+1 \in [a,b]\}
$$
3. 
$$
T(C(S)) = \{x \in X^{\mathbb{Z}-1} | x_i \in S_{i+1}, \forall i + 1 \in [a,b], x_j \in X \forall j \notin [a,b]\} =  \{x \in X^{\mathbb{Z}}| x_i \in S_{i+1}, \forall i + 1 \in [a, b]\} = C(D(S))
$$
*cqd*
...
v_n(S)=v_{n+1}(T(S) \times X), \quad \text{(T.3)}
$$
